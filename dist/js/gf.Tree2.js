  ...
  $.when($.ajax({
    ...
    gfTree = function gfTree(element, options) {
      ...
      if (initResult) {
        ...
        this._subscribeEvents(); //如果有設定預設開啟物件
    ...
    gfTree.prototype = {
      ...
      _style: function _style() {
        ...
        o.target.append(toolbar); //子容器

        var subContainer = $('<div/>').appendTo(o.target);
        subContainer.height(o.target.height() - toolbar.outerHeight()); //歷史圖層
        ...
        searchResultList.height(o.target.height() - toolbar.outerHeight() - searchInput.outerHeight()); //圖層清單容器

        var itemlist = $('<div/>', {
          'class': 'gfTreeItemList'
        }).appendTo(subContainer);
        itemlist.height(o.target.height() - toolbar.outerHeight()); //產生圖層清單並塞到容器中

        o.opt.arrData.filter(function (x) {
      ...
      _event: function _event() {
        ...
        o.target.on('click', '.gfTreeItem', function (e) {
          var et = $(this);
          var eid = et.data().id;
          var lvl = et.data().lvl;
          var st = et.data().st;
        ...
        o.target.on('click', '.gfTreeToolbar-Icon[data-type="search"]', function () {
          o.target.find('.gfTreeSearchContainer').show().siblings().hide();
        });
        o.target.on('keyup', '.gfTreeSearchInput', function (e) {
          var code = e.which;
          ...
          if (code == 32 || code == 13 || code == 188 || code == 186) {
            ...
            o.target.find('.gfTreeSearchResultList').html(resultDiv);
            o.target.find('.gfTreeSearchResultList').getNiceScroll().resize();
          }
        }); //工具 - 回到圖層清單

        o.target.on('click', '.gfTreeToolbar-Icon[data-type="list"]', function () {
          o.target.find('.gfTreeItemList').show().siblings().hide();
        }); //工具 - 歷史圖層
        // o.target
      ...
      _cssTheme: function _cssTheme(theme) {
      ...
      _contextMenuAction: function _contextMenuAction(_action, _value) {
        var o = this;
      ...
      _subscribeEvents: function _subscribeEvents() {
        ...
        this.target.off('onSetFlyto'); //綁定點擊事件接口
      ...
      _getSelector: function _getSelector(jqueryObj) {
        var selector = jqueryObj.parents().map(function () {
          return this.tagName;
